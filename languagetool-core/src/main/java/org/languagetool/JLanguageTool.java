...
*/
  ...
  protected List<RuleMatch> performCheck(List<AnalyzedSentence> analyzedSentences, List<String> sentences,
    ...
    return performCheck(analyzedSentences, sentences, allRules, paraMode, annotatedText, null, mode, level);
  }
   ...
   */
  protected List<RuleMatch> performCheck(List<AnalyzedSentence> analyzedSentences, List<String> sentences,
                                         List<Rule> allRules, ParagraphHandling paraMode, AnnotatedText annotatedText, RuleMatchListener listener, Mode mode, Level level) throws IOException {
    Callable<List<RuleMatch>> matcher = new TextCheckCallable(allRules, sentences, analyzedSentences, paraMode, annotatedText, 0, 0, 1, listener, mode, level);
    try {
      return matcher.call();
    } catch (IOException e) {
      throw e;
  ...
  class TextCheckCallable implements Callable<List<RuleMatch>> {
    ...
    TextCheckCallable(List<Rule> rules, List<String> sentences, List<AnalyzedSentence> analyzedSentences,
                      ...
                      RuleMatchListener listener, Mode mode, Level level) {
      this.rules = rules;
