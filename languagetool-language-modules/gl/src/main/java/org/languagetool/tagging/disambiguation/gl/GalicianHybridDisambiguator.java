...
public class GalicianHybridDisambiguator extends AbstractDisambiguator {
    ...
    private final Disambiguator chunker = new MultiWordChunker("/gl/multiwords.txt", true, true);
    ...
    while (i < aTokens.length) {
      if (!aTokens[i].isWhitespace()) {
        if (!nextPOSTag.isEmpty()) {
          ...
          aTokens[i] = new AnalyzedTokenReadings(aTokens[i], Arrays.asList(newAnalyzedToken), "GL_HYBRID_DISAMB_END");
          ...
          aTokens[i] = new AnalyzedTokenReadings(aTokens[i], Arrays.asList(newAnalyzedToken), "GL_HYBRID_DISAMB_START");
          if (POSTag.startsWith("NC")) {
            nextPOSTag = "AQ0" + POSTag.substring(2, 4) + "0_";
          } else {
            nextPOSTag = POSTag;
          }
        }
