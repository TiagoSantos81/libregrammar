 ...
 * @since 4.8
 */
public class PortugueseSpecificCaseRule extends Rule {
  
  private static final Set<String> phrases = new HashSet<>(loadPhrases("/pt/specific_case.txt"));
  private static int maxLen;

  private static List<String> loadPhrases(String path) {
    List<String> l = new ArrayList<>();
    InputStream file = JLanguageTool.getDataBroker().getFromRulesDirAsStream(path);
  ...
  public PortugueseSpecificCaseRule(ResourceBundle messages) {
    super(messages);
  ...
  @Override
  public URL getUrl() {
    return Tools.getUrl("https://pt.wikipedia.org/wiki/Caixa_alta_e_caixa_baixa");
  }
    ...
    for (int i = 0; i < tokens.length; i++) {
      ...
      while (l.size() < maxLen && i+j < tokens.length) {
        ...
        if (properSpelling != null && !StringTools.isAllUppercase(phrase) && !phrase.equals(properSpelling)) {
          String msg = "Esta expressão, por norma, utiliza uma capitalização distinta.";
